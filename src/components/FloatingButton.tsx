    ...
    <div className={`fixed z-50 transition-all duration-500 ease-in-out ${
      ...
      <div className={`flex flex-col-reverse items-end mb-3 space-y-reverse space-y-2 transition-all duration-500 ${isOpen ? 'opacity-100 scale-100' : 'opacity-0 scale-95 pointer-events-none'}`}>
        <a 
          ...
          className="flex items-center bg-white text-primary px-3 sm:px-4 py-2.5 sm:py-3 rounded-lg shadow-lg hover:shadow-xl border border-primary/20 hover:bg-primary/5 transition-all duration-300 gap-2 sm:gap-3 font-medium group text-xs sm:text-sm md:text-base whitespace-nowrap" 
      ...
      <button 
        ...
        className={`${
          isMobile 
            ? 'p-3 w-12 h-12' 
            : 'p-3 w-12 h-12 sm:p-4 sm:w-14 sm:h-14 md:p-5 md:w-16 md:h-16'
        } rounded-full flex items-center justify-center transition-all duration-300 
          bg-primary hover:bg-primary/90 text-white
          ${isPulsing ? 'animate-pulse' : ''}
          transform hover:scale-105 active:scale-95 shadow-lg hover:shadow-xl min-h-[44px] min-w-[44px]`} 
        aria-label="Contatar via WhatsApp"
        style={{
          boxShadow: '0 8px 25px rgba(20, 184, 166, 0.4)'
        }}
      >
        <MessageCircle size={isMobile ? 18 : 20} className="sm:w-5 sm:h-5 md:w-6 md:h-6" />
      </button>
    </div>
  );
};
