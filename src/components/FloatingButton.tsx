    ...
    <div className={`fixed z-50 ${
      ...
      <div className={`flex flex-col-reverse items-end mb-3 space-y-reverse space-y-2 transition-all duration-500 ${isOpen ? 'opacity-100 scale-100' : 'opacity-0 scale-95 pointer-events-none'}`}>
        <a 
          ...
          className="flex items-center bg-white text-primary px-4 py-3 rounded-lg shadow-lg hover:shadow-xl border border-primary/20 hover:bg-primary/5 transition-all duration-300 gap-3 font-medium group text-sm md:text-base whitespace-nowrap" 
      ...
      <button 
        ...
        className={`${
          isMobile 
            ? 'p-3 w-12 h-12' 
            : 'p-4 w-14 h-14 md:p-5 md:w-16 md:h-16'
      ...
      >
        <MessageCircle size={isMobile ? 18 : 20} className="md:w-6 md:h-6" />
      </button>
    </div>
  );
};
